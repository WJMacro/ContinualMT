...
class JSDivergence(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        if sample["net_input"]["src_tokens"].size(0) == sample["target"].size(0):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        loss = compute_js_loss(model, net_output, pad_mask)
...
def compute_js_loss(model, net_output, pad_mask=None, reduce=True):
